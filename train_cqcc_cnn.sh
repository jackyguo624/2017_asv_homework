...
GPUS=`nvidia-smi|awk 'BEGIN{n=0}{if(NF==15){ print n,$11-$9;n=n+1  }}'| sort -k 2 -n -r|awk '{print $1}'| head -n 2`
gpus_str=`echo $GPUS`
...
/slwork/users/jqg01/tools/anaconda2/bin/python src/train_new.py "$TRAINDATA" "$TR_LABELS" "$CVDATA" "$CV_LABELS" -model "cnn_cqcc"
