...
def getDataloader(path, model, label, mode):
    ids, feats = (None, None)
    print (model)
    idform = IDforamter(mode)
    print (model)
    if 'cqcc' in model:
        ids, feats = zip(*[(idform.hash(f),
                            norm_arr(sio.loadmat(join(path, f))
                                     ['CQcc'][0:20, 0:200]))
                           for f in tqdm(listdir(path)) if f in label])

    elif 'mfcc' in model or 'fbank' in model or 'fft' in model:
        ids, feats = zip(*[(idform.hash(key), norm_arr(mat.T[:, 0:200]))
...
def main():
    ...
    evalloader, inputdim = getDataloader(args.evaldata, args.model,
                                         evallabel, mode=args.mode)
